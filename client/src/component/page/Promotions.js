...
import { Button, Table, Switch, Modal, Tooltip, Badge, Descriptions, Divider, notification, Tag } from 'antd';
...
import { useConfigCache, useOrdersQuery, useProductsQuery } from '../../utils/customHook';
import { rewardTypeOptions, getAllProductCategory } from '../../utils/Constants';
...
const Promotions = (props) => {
  ...
  const { data: productsResult, loading: loadingProducts} = useProductsQuery({
    variables: {
      configId: configId
    },
  ...
  const productCategoryTags = (selectedCategories=[]) => {
    let products = productsResult && productsResult.products ? productsResult.products : [];
    let categories = getAllProductCategory(products);

    let result = [];

    selectedCategories.forEach((aSelectedCategory,index)=>{
      let foundCategory = categories.find((aCategory)=>aCategory._id == aSelectedCategory)
      if (foundCategory) {
        result.push(<Tag key={index} style={{margin: '4px 2px'}}>{foundCategory.name}</Tag>)
      }
    })
  ...
  const productTags = (selectedProducts=[]) => {
    let products = productsResult && productsResult.products ? productsResult.products : [];

    let result = [];

    selectedProducts.forEach((aSelectedProduct,index)=>{
      let foundProduct = products.find((aProduct)=>aProduct._id == aSelectedProduct)
      if (foundProduct) {
        result.push(<Tag key={index} style={{margin: '4px 2px'}}>{foundProduct.name}</Tag>)
      }
    })
  ...
  let isLoading = loadingPromotions || updatePublishLoading || deleteLoading || loadingProducts
          ...
          selectedPromotion ? (
            <>
              <Descriptions bordered column={{ xxl: 2, xl: 2, lg: 2, md: 2, sm: 1, xs: 1 }}>
                ...
                <Descriptions.Item label="Description">{selectedPromotion.description ? selectedPromotion.description : '-'}</Descriptions.Item>
              </Descriptions>
              <Divider orientation="left">Conditions</Divider>
              <Descriptions bordered column={{ xxl: 2, xl: 2, lg: 2, md: 2, sm: 1, xs: 1 }}>
                <Descriptions.Item label="Categories">{productCategoryTags(selectedPromotion.categories)}</Descriptions.Item>
                <Descriptions.Item label="Products">{productTags(selectedPromotion.products)}</Descriptions.Item>
                ...
                <Descriptions.Item label="Minimum Weight (g)">{selectedPromotion.minWeight ? selectedPromotion.minWeight : '-'}</Descriptions.Item>
              </Descriptions>
