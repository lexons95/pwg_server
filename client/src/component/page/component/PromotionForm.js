...
import { Button, Form, Input, Switch, InputNumber, DatePicker, Select, Divider, Row, Col, Tooltip, Modal, Space, notification } from 'antd';
...
const PromotionForm = (props) => {
  ...
  const [ createPromotion, { loading: loadingCreatePromotion, data: createPromotionResult }] = useMutation(CREATE_PROMOTION_MUTATION,{
    onCompleted: (result) => {
      ...
      if (result && result.createPromotion && result.createPromotion.success) {
    ...
    <div style={modalContentWrapperStyle}>
      ...
      >
          ...
          <Col xs={24} md={8}>
            <Form.Item 
              ...
              label={(
                <span>
                  Minimum Purchases&nbsp;
                  <Tooltip title="Minimum subtotal of selected categories/products">
                    <QuestionCircleOutlined />
                  </Tooltip>
                </span>
              )}
              rules={[
                ({ getFieldValue }) => ({
                  validator(rule, value) {
                    ...
                    let minWeight = getFieldValue('minWeight');
                    if (value || minQuantity || minWeight) {
                      return Promise.resolve();
                    }
                    return Promise.reject('Please enter at least one of the fields (Minimum Purchases/Minimum Quantity/Minimum Weight)');
                  },
                }),
              ]}
            >
              <InputNumber style={{width: '100%'}} placeholder="Leave blank if not applicable"/>
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
            <Form.Item 
              ...
              label={(
                <span>
                  Minimum Quantity&nbsp;
                  <Tooltip title="Minimum total quantity of selected categories/products">
                    <QuestionCircleOutlined />
                  </Tooltip>
                </span>
              )}
              rules={[
                ({ getFieldValue }) => ({
                  validator(rule, value) {
                    ...
                    let minWeight = getFieldValue('minWeight');
                    if (value || minPurchases || minWeight) {
                      return Promise.resolve();
                    }
                    return Promise.reject('Please enter at least one of the fields (Minimum Purchases/Minimum Quantity/Minimum Weight)');
                  },
          ...
          <Col xs={24} md={8}>
            <Form.Item 
              ...
              label={(
                <span>
                  Minimum Weight (g)&nbsp;
                  <Tooltip title="Minimum total weight of selected categories/products">
                    <QuestionCircleOutlined />
                  </Tooltip>
                </span>
              )}
              rules={[
                ({ getFieldValue }) => ({
                  validator(rule, value) {
                    let minQuantity = getFieldValue('minQuantity');
                    let minPurchases = getFieldValue('minPurchases');
                    if (value || minQuantity || minPurchases) {
                      return Promise.resolve();
                    }
                    return Promise.reject('Please enter at least one of the fields (Minimum Purchases/Minimum Quantity/Minimum Weight)');
                  },
        ...
        <Row gutter={24}>
          ...
          <Col xs={24} md={12}>
            {
              rewardTypeValue == "percentage" || rewardTypeValue == "fixedAmount" || rewardTypeValue == "charges" ? (
